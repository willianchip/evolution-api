...
serve(async (req) => {
  ...
  try {
    ...
    const facebookSecret = Deno.env.get('FACEBOOK_APP_SECRET')!;
    const supabase = createClient(supabaseUrl, supabaseKey);

    const url = new URL(req.url);
    
    // Webhook do Facebook (GET para verificação)
    if (req.method === 'GET' && url.pathname.includes('/webhook')) {
      const mode = url.searchParams.get('hub.mode');
      const token = url.searchParams.get('hub.verify_token');
      const challenge = url.searchParams.get('hub.challenge');

      if (mode === 'subscribe' && token === facebookSecret) {
    ...
    // Webhook do Facebook (POST para mensagens)
    ...
    if (req.method === 'POST' && url.pathname.includes('/send')) {
      ...
      if (result.message_id) {
        ...
        console.log('Facebook message sent:', result);
